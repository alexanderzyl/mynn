...
# assert torch.cpu.is_available(), 'You need to have an Nvidia GPU with CUDA installed.'
...
class StyleGAN2(nn.Module):
    def __init__(self, image_size, latent_dim=512, fmap_max=512, style_depth=8, network_capacity=16, transparent=False,
                 cl_reg=False, steps=1, ttur_mult=2, fq_layers=[], fq_dict_size=256,
                 attn_layers=[], no_const=False, lr_mlp=0.1):
        super().__init__()
        self.steps = steps
        ...
        self.G_opt = Adam(generator_params, betas=(0.5, 0.9))
        self.D_opt = Adam(self.D.parameters(), betas=(0.5, 0.9))
...
class Trainer():
    ...
    def init_GAN(self):
        ...
        self.GAN = StyleGAN2(ttur_mult=self.ttur_mult, image_size=self.image_size,
                             ...
                             attn_layers=self.attn_layers, cl_reg=self.cl_reg, no_const=self.no_const,
                             *args, **kwargs)
    ...
    @torch.no_grad()
    def evaluate(self, num, num_image_tiles):
        self.GAN.eval()
    ...
    def truncate_style(self, tensor, trunc_psi=0.75):
        ...
        av_torch = torch.from_numpy(self.av).cpu()
    ...
    def load(self, num=-1):
        ...
        try:
            ...
            print(
                'unable to load save model. please try downgrading the package to the version specified by the saved '
                'model')
            raise e
